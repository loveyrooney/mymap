...
async def call_subway(st_name:str)-> list:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
            ...
            if response.status == 200:
                ...
                print(f"api 받은 시각: {int(time.time() * 1000)}")
                #print(f"itemLists===> {itemLists}")
                return [subway_filter(item,"상행") for item in itemLists if subway_filter(item,"상행")]
            else:
                return response.status
            
# bike filter
BIKE_KEYSET = {}
def bike_filter(item: dict, direction: str) -> dict:
    if 'updnLine' in item and item['updnLine'] == direction:
        return {k: item[k] for k in item if k in SUB_KEYSET}
    return {}
